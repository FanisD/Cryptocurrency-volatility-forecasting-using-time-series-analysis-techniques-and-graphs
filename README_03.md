...
### Διευκρίνιση Σχετικά με τα 3 Datasets
Στο παρόν πείραμα (καθώς και στα ST-GNNs που ακολουθούν) χρησιμοποιούνται τρία ξεχωριστά datasets. Είναι σημαντικό να τονιστεί γιατί δεν ενοποιήθηκαν σε ένα:

1. **Yahoo Finance (Το Κύριο Dataset):** Διαθέτει ~2.986 ημέρες δεδομένων. Χρησιμοποιείται ως το βασικό σύνολο εκπαίδευσης (Training Set) καθώς προσφέρει το μεγαλύτερο ιστορικό βάθος για να μάθουν τα μοντέλα τις μακροχρόνιες συσχετίσεις.
2. **Binance (~1.000 ημέρες) & Coinbase (~350 ημέρες):** Αυτά τα datasets λειτουργούν αποκλειστικά ως σύνολα ελέγχου ανθεκτικότητας (Robustness / Out-of-Distribution Sets). Τα διατηρούμε αυστηρά διαχωρισμένα για να αξιολογήσουμε αν το μοντέλο, αφού εκπαιδευτεί στο Yahoo, μπορεί να γενικεύσει σε νέα δεδομένα από πραγματικά ανταλλακτήρια.
* Σημείωση: Ο ρυθμός δειγματοληψίας είναι κοινός (Ημερήσιος). Η διαφοροποίηση στα αποτελέσματα οφείλεται στο διαφορετικό χρονικό παράθυρο που καλύπτει το καθένα.
